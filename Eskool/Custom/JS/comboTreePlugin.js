    ...
    $.fn.DropDownTree = function (options) {
        ...
        $(options.element).on("click", ".select-box", function (e) {
            ...
            /*    $(this).parents("li").first().addClass("selected");*/
              ...
              /*      $(this).parents("li").first().find(".select-box").parents("li").first().addClass("selected");*/
                }
            } else {
                //will unselect
                checked = false;
              ...
              /*  $(this).parents("li").first().removeClass("selected");*/
                if (options.selectChildren) {
                   ...
                   /* $(this).parents("li").first().find(".select-box").parents("li").first().removeClass("selected");*/
                    $(this).parents("li").each(function () {
                        $(this).find(".select-box").first().removeClass("fa-check-square-o");
                        $(this).find(".select-box").first().addClass("fa-square-o");
     /*                   $(this).find(".select-box").first().parents("li").first().removeClass("selected");*/
                    });
                }
            }
            ...
            $(options.element).find(".fa-check-square-o").each(function () {
                $(this).parents("li").first().addClass("selected");
            });
            $(options.element).find(".fa-square-o").each(function () {
                $(this).parents("li").first().removeClass("selected");
            });
        });
        ...
        if (options.isExpanded == false) {
            ...
            $(options.element).append('<ul style="max-height: ' + options.maxHeight + 'px; position: absolute; transform: translate3d(0px, 32px, 0px); top: 0px; left: 0px; will-change: transform;" class="dropdown-menu show" aria-labelledby="dropdownMenu1"></ul>');
        }
        ...
        $(options.element).init.prototype.GetSelected = function (title) {
            ...
            $(this).find(".fa-check-square-o").each(function () {
                ...
                selectedElements.push($(this).parents("li").first().attr("data-Value"));
            });
